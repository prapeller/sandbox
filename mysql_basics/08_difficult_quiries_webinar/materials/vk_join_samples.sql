...
FROM users
         JOIN friend_requests
              ON (users.id = friend_requests.initiator_user_id OR users.id = friend_requests.target_user_id)
...
-- кому я отправлял заявку в друзья
SELECT m.*
FROM media m
...
UNION
 -- кто мне отправлял заявку в друзья
select m.*
FROM media m
